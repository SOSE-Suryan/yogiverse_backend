...
class PostReelListAPIView(generics.ListAPIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        posts = Post.objects.filter(user=user,is_draft=False).order_by('-created_at')
        reels = Reel.objects.filter(user=user,is_draft=False).order_by('-created_at')
